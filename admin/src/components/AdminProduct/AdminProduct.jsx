    ...
    },
    {
        ...
        render: (_, product) => (<>
            ...
            <Popconfirm
                ...
                onConfirm={() => handleOkeDelete(product.id)}
                onCancel={() => { }}
                okText="Yes"
                cancelText="No"
            >
...
export default function AdminProduct() {
    ...
    const getProducts = async () => {
        const result = await axios.get("http://localhost:7500/products")
    ...
    const onFinish = async (values) => {
        ...
        if (productUpdate) {
            ...
            // gửi dữ liệu lên db
            const result = await axios.put(`http://localhost:7500/products/${productUpdate.id}`, newProduct)
        ...
        if (fileImage) {
            ...
            if (url) {
                ...
                const result = await axios.post("http://localhost:7500/products", newProduct)
    ...
    const handleOkeDelete = async (id) => {
        const result = await axios.delete(`http://localhost:7500/products/${id}`)
        if (result.status == 200) {
    ...
    const handleClickSearch = async (value) => {
        const result = await axios.get(`http://localhost:7500/products?name_like=${value}`)
